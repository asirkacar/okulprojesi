...
#include <UI/VeriFormlari/tknmusteripencere.h>
...
#include <UI/ListeFormlari/tknmusteriliste.h>

#include <UI/VeriFormlari/tkntedarikcipencere.h>
...
void TKNAnaPencere::on_actionYeni_M_teri_triggered()
{
    TKNMusteriPencere form;
    auto veri=TKNGenelVeriYoneticisi::sec().getMusteriBilgileri().yeni();

    form.setVeri(veri);
    form.setWindowTitle(tr("Yeni Müşteri Girişi"));
    if(form.exec()==QDialog::Accepted){
        veri = form.getVeri();
        TKNGenelVeriYoneticisi::sec().getMusteriBilgileri().ekle(veri);}
}
